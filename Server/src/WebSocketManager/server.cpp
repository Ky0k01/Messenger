...
void Server::readSaltFile(){
    QString filePath=QCoreApplication::applicationDirPath()+"/salt.txt";
    QFile file(filePath);
    ...
    if(file.open(QIODevice::ReadOnly | QIODevice::Text)){
        ...
        qCritical()<<"Salt file not opened!";
        return;
    }
}
...
void Server::onNewConnection() {
    while(server->hasPendingConnections()){
        ...
        connect(socket, &QWebSocket::pong, this, &Server::onPongReceived);
...
void Server::onPongReceived(quint64 elapsedTime, const QByteArray& payLoad){
